...
module.exports = async function generateNativeCode() {
  ...
  var partials = {
    ...
    callbackHelpers: utils.readLocalFile(
      "templates/partials/callback_helpers.cc"
    ),
    configurableCallbacks: utils.readLocalFile(
      "templates/partials/configurable_callbacks.cc"
    ),
    ...
    guardArguments: utils.readLocalFile(
      "templates/partials/guard_arguments.cc"
    ),
    syncFunction: utils.readLocalFile("templates/partials/sync_function.cc"),
    fieldAccessors: utils.readLocalFile(
      "templates/partials/field_accessors.cc"
    ),
  ...
  var templates = {
    ...
    struct_content: utils.readLocalFile(
      "templates/templates/struct_content.cc"
    ),
  ...
  Object.keys(partials).forEach(function (partial) {
    ...
    templates.class_content.registerPartial(
      partial,
      combyne(partials[partial])
    );
    templates.struct_header.registerPartial(
      partial,
      combyne(partials[partial])
    );
    templates.struct_content.registerPartial(
      partial,
      combyne(partials[partial])
    );
  });
  ...
  const enabled = idefs.filter(function (idef) {
    return !idef.ignore;
  });
  ...
  try {
    ...
    // fs.cpSync is not yet fully supported, so we must use fs-extra.
    await fse.copy(
    ...
    try {
      if (await exec("command -v astyle")) {
        await exec(
      ...
      console.log("`command -v astyle` failed, skipping astyle");
    }
